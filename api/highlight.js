async function Highlight() {
  ...
  globalThis.asunc = async function() {
  ...
  setInterval(function() {
    if (document.querySelector('html').hasAttribute('location')) {
      ...
      if (document.querySelector('html').getAttribute('location') != window.location.href) {
        location.reload();
        document.querySelector('html').setAttribute('location', window.location.href);
      }
  ...
  async function stripCodes() {
    ...
    for (let i = 0; i < doublecodes_length; i++) {
      try {
        await asunc();
  ...
  async function getp() {
    ...
    try {
      if (Prism && (!(globalThis.Prasm))) {
        globalThis.Prasm = Prism;
      }
    } catch (e) {}
  ...
  [class="font-mono"]:not([highlighted])`);
    ...
    for (let i = 0; i < faces_length; i++) {
      try {
        await asunc();
        if (faces[i].outerHTML.toString().includes('highlight-me')) {
          highlighter();
          continue;
        }
        ...
        if (faces[i].getAttribute('class').includes('tw-') || faces[i].getAttribute('class').includes('font-mono')) {
    ...
    for (let i = 0; i < codes_length; i++) {
      await asunc();
      if (codes[i].innerHTML.toString().includes('highlight-me')) {
        highlighter();
        continue;
      }
      let hlc = 0;
      ...
      if (preclass && preclass.includes('language-')) {
        mylang = ('language-' + preclass.split('language-')[1].split(' ')[0])
          ...
          .replace('-c"', '-clike"')
          .replace('-c ', '-clike ')
          .replaceAll(',', ''); //.replace('shell','docker');
      }
    ...
    async function addScript(s) {
      ...
      if (document.querySelector('script[id="' + s + '"]')) {
        return;
      }
      jsx.id = s;
      let aspromise = new Promise(resolve => {
        jsx.resolve = resolve
      })
      jsx.src = s;
      jsx.onload = function() {
